...
int main(int argc, char **argv) {
    ...
    // // BAS
    // float (*dist) (t_vvalue, t_vvalue) = d2v;
    // Samples<t_vvalue> seqs;
    // seqs.setVectorSize(60);
    // seqs.read("mpeg7bbr_s-cchd100_f-bas.vectors");

    // // Shape contexts
    // float (*dist) (t_vvalue, t_vvalue) = chiSquared;
    // Samples<t_vvalue> seqs;
    // seqs.setVectorSize(60);
    // seqs.read("mpeg7bbr_s-cchd100_f-sc.vectors");
    ...
    for (unsigned i=0; i<seqs.samples->size(); ++i) {
        ...
        for (unsigned j=0; j<seqs.samples->size(); ++j) {
            if (i == j) continue;
            float cost = maes((*seqs.samples)[i],
                              (*seqs.samples)[j], dist);
            if (cost < min_value) {
                min_value = cost;
                min_j = j;
